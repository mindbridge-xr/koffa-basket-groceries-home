...
const Onboarding = () => {
  ...
  const steps = [{
    ...
    content: <div className="text-center space-y-6">
          <div className="w-32 h-32 bg-gradient-primary rounded-3xl flex items-center justify-center mx-auto">
            <span className="font-bold text-white font-poppins text-7xl">🧺</span>
          </div>
    ...
    content: <div className="space-y-6">
          ...
          desc: "Collaborative grocery and task lists"
        }, {
          icon: Calendar,
          title: "Family Schedule",
          desc: "Coordinate events and activities"
        }, {
          icon: ChefHat,
          title: "Chef Services",
          desc: "Book professional cooking services"
        }, {
          icon: Users,
          title: "Family Circle",
          desc: "Manage family members and roles"
        }].map((feature, index) => <div key={index} className="card-familyhub p-4">
    ...
    content: <div className="space-y-6">
          ...
          <div className="space-y-4">
            <Input placeholder="e.g., The Johnson Family" value={familyName} onChange={e => setFamilyName(e.target.value)} className="input-familyhub text-center text-lg" />
  ...
  return <div className="min-h-screen bg-gradient-to-br from-primary/5 via-secondary/5 to-accent/5 flex flex-col">
      {/* Progress */}
      <div className="mobile-padding">
        <div className="flex justify-between items-center mb-8">
          <div className="flex space-x-2">
            {steps.map((_, index) => <div key={index} className={`h-2 w-8 rounded-full transition-colors ${index <= currentStep ? 'bg-primary' : 'bg-gray-200'}`} />)}
          </div>
      ...
      <div className="mobile-padding">
        <div className="flex space-x-4">
          {currentStep > 0 && <Button variant="outline" onClick={() => setCurrentStep(currentStep - 1)} className="btn-secondary flex-1">
              Back
            </Button>}
          <Button onClick={handleNext} disabled={currentStep === steps.length - 1 && !familyName.trim()} className="btn-primary flex-1">
